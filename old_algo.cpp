...
int	check_base_danger(vector <t_entity> *list, t_general *general)
{
	for (int i = 0; i < list->size(); i++)
	{
...
void	action_algo(t_general *general, int hero)
{
	vector <t_entity>	*list;

	list = _entity();
	calculate_threat_all(list, general);
	if (hero == 0)
		hero_0(list, general, hero);
	else if (hero == 1)
		hero_1(list, general, hero);
	else if (hero == 2)
		hero_2(list, general, hero);
}
